...
public class TestConsumer {
     ...
     */
    @RabbitListener(bindings = @QueueBinding(
            value = @Queue(value = "${delay.queue.value}",durable = "${delay.queue.durable}"),
            exchange = @Exchange(name = "${delay.exchange.name}",durable = "${delay.exchange.durable}"
                    ,type = "${delay.exchange.type}",ignoreDeclarationExceptions = "${delay.exchange.ignoreDeclarationExceptions}"),
            key = "${delay.key}"))
    @RabbitHandler
